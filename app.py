...
app.config['MYSQL_PASSWORD']='manumadhira@'
...
def edit():
    ...
    if session.get('admin'):
        if request.method=='POST':
            ...
            To_date=request.form['To']
            cursor=mysql.connection.cursor()
            cursor.execute('update notifications set Fileupload=%s,notifname=%s,From_date=%s,To_date=%s where notifid=%s',[File.read(),name,From_date,To_date,id1])
            mysql.connection.commit()
            cursor.close()
            return redirect(url_for('adminpanel',adminid=session['admin']))
